...
def beautiful_output(k, r, c, r_z, c_z, mat, new_mat, elem, List_res):
    ...
    if k == 0 and r[0] == r_z[0] and c[0] == c_z[0]:
    ...
    if r[0] == r_z[0] and c[0] == c_z[0] and elem[0] != 0:
        print(elem[0])
    ...
    if r[0] == r_z[0] and c[0] == c_z[0] and k == len(mat) - 1:
    ...
    if (len(mat) == 1):
        ...
        if (r[1] > c[1] or r[1] == c[1]):
            for k in range(len(mat[r[0]])):
                ...
                # if we go into recursion 1 time then we create a global list that will store the results of the minors
                if k == 0 and r[0] == r_z[0] and c[0] == c_z[0]:
                    global List_res
                    List_res = []
                beautiful_output(k, r, c, r_z, c_z, mat, new_mat, elem, List_res)
...
with open(name_file) as file:
    ...
    global r_z, c_z # need to definitions of the first step of entering recursion
    r_z, c_z = zero_row(matrix), zero_column(matrix)
